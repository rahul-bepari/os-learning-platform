  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        borderBottom: '1px solid ' + ch.color + '44',
        padding: '48px 60px'
      }}>
        <div style={{
          fontSize: 12, color: ch.color, fontWeight: 600,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 12 }}>
          ...
          <h1 style={{ fontSize: 40, fontWeight: 800, color: 'var(--text-primary)' }}>
            {ch.title}
          </h1>
        </div>
      ...
      <div style={{ padding: 60 }}>
        <div style={{
          ...
          <div style={{ fontSize: 15, marginBottom: 8 }}>
            Building Chapter {ch.id}:{' '}
            <strong style={{ color: ch.color }}>{ch.title}</strong>
          </div>
          <div style={{ fontSize: 14, color: 'var(--text-muted)', marginTop: 8 }}>
            Will include detailed explanations, visualizations,
            code editor, terminal, and quiz — just like Chapters 1 to 5.
          </div>
          <div style={{
            marginTop: 32,
            ...
            <Link to={'/chapter/' + prev.id} style={{ textDecoration: 'none' }}>
              ...
              >
                ...
                <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>
                  {prev.icon} Ch {prev.id}: {prev.title}
                </div>
              </div>
            </Link>
          )
        ...
        {next && (
          <Link to={'/chapter/' + next.id} style={{ textDecoration: 'none' }}>
            ...
            >
              ...
              <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>
                {next.icon} Ch {next.id}: {next.title}
              </div>
            </div>
          </Link>
        )}
